...
class ServerServeE2ETests(unittest.TestCase):
    ...
    def test_train_eval_promote_and_status_serve_distinguish_latest_from_recent(self) -> None:
        ...
        promote_result = store.promote(first_result.version)
        self.assertIn(first_result.version, promote_result)
        ...
        with patch.dict(sys.modules, {"uvicorn": fake_uvicorn}):
            ...
            self.assertEqual(body["metadata"]["lifecycle"]["promotion"]["last_real_execution_state"], body["metadata"]["real_execution"]["state"])
            self.assertEqual(
                body["metadata"]["lifecycle"]["promotion"]["last_export_toolchain_status"],
                body["metadata"]["export_toolchain"]["toolchain_status"],
            )
