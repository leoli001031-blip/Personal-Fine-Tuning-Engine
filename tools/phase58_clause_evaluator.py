#!/usr/bin/env python3
"""Run the frozen Phase58 clause-addressed protocol with two local judges."""
...
def _judge_one(
    *, item: Mapping[str, Any], alias: str, model: str, endpoint: str, timeout: int, protocol: Mapping[str, Any]
    ...
    clause_ids = [row["clause_id"] for row in segment_phase58_clauses(item.get("assistant_response"))]
    schema = phase58_ollama_json_schema(clause_ids)
    ...
    extraction = validate_phase58_raw_extraction(parsed, clause_ids=clause_ids)
    ...
    return {
        ...
        **extraction,
        ...
        "gold_clause_ids_hidden_from_judge": True,
...
def _freeze_check(split: str, public: list[dict[str, Any]], hidden: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    source_ok = (
        ...
        and freeze.get("phase58_evaluator_source_sha256") == _sha256(PHASE58_SOURCE)
    )
    protocol_ok = (
    ...
    if split == "holdout":
        prerequisite_ok = _read_json(
            EVIDENCE_ROOT / "evidence-evaluator-calibration/clause_evaluator_report.json"
        ).get("status") == "qualified"
    return {
        "kind": "phase58_clause_evaluator_freeze_check",
        "split": split,
...
def main() -> int:
    ...
    if freeze_check["passed"] is not True:
        raise SystemExit(f"Phase58 {args.split} freeze check failed: {freeze_check}")

    protocol = _read_json(EVIDENCE_ROOT / "evaluator_protocol.json")
    workers = int(protocol.get("parallel_worker_count") or 1)
    if args.workers is not None and args.workers != workers:
        raise SystemExit(f"Phase58 worker count is frozen at {workers}; received --workers={args.workers}")
    ...
    availability = {
        "kind": "phase58_judge_model_availability",
        "split": args.split,
    ...
    if availability["passed"] is not True:
        raise SystemExit(f"Phase58 judge models unavailable: {missing}")

    all_results = []
    failures = []
    for alias in aliases:
        output_path = directory / f"judge_clause_results_{alias}.jsonl"
        if output_path.exists() and not args.resume:
            raise SystemExit(f"Phase58 {args.split} is frozen one-shot evidence; use --resume only for an interrupted run")
        ...
        for row in existing:
            ...
            clause_ids = [entry["clause_id"] for entry in segment_phase58_clauses(item.get("assistant_response"))]
            try:
                validate_phase58_raw_extraction(row, clause_ids=clause_ids)
            except ValueError:
                continue
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                print(
                    ...
                    f"relation={result['test_to_user_outcome_relation']} latency={result['latency_seconds']}s",
                    flush=True,
                )
    ...
    _write_json(directory / "clause_evaluator_report.json", report)
    print(
        json.dumps(
            {key: report.get(key) for key in (
                "split", "status", "item_count", "completed_item_count", "accuracy",
                "typed_exact_match_rate", "grounding_validity_rate", "invalid_atom_count",
                "invalid_dangerous_atom_count", "composer_received_ungrounded_atom_count",
                "per_field", "per_category", "false_accept_count_on_reject_cases", "failure_count",
            )},
            ensure_ascii=False,
            indent=2,
        )
