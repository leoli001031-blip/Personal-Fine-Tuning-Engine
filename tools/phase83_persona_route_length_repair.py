#!/usr/bin/env python3
"""Run the Phase83 persona route and output-length repair benchmark."""
...
EVIDENCE_ROOT = REPO_ROOT / "docs/demo/phase83-persona-route-length-repair"
...
CORE_SOURCE = CORE_ROOT / "pfe_core/phase83_persona_route_length_repair.py"
DRIVER_SOURCE = REPO_ROOT / "tools/phase83_persona_route_length_repair.py"
TEST_SOURCE = REPO_ROOT / "tests/test_phase83_persona_route_length_repair.py"
...
GENERATION_PROTOCOL = {
    "kind": "phase83_frozen_persona_api_v2_generation_protocol",
    ...
    "persona_response_contract": "contract_persona_guarded_v2",
...
def _prepare(clean: bool) -> int:
    ...
    isolation = audit_phase83_isolation(holdout["sessions"], _previous_holdouts())
    ...
    checks = {
        ...
        "holdout_count_30": holdout.get("session_count") == PHASE83_SESSION_COUNT,
        "no_training_run_planned": True,
        "product_default_unchanged": True,
    }
    freeze = {
        "kind": "phase83_pre_experiment_freeze",
        "created_at": _utcnow(),
    ...
    _write_json(EVIDENCE_ROOT / "preparation_decision.json", {
        "kind": "phase83_preparation_decision",
...
def _freeze_check() -> dict[str, Any]:
    ...
    return {"kind": "phase83_generation_freeze_check", "passed": all(checks.values()), "checks": checks}
...
def _run_session(
    *,
    ...
    persona = variant == PHASE83_VARIANTS[1]
    ...
    for turn, raw_user in enumerate(
        ...
        payload = service.chat_completion(
            ...
            metadata=metadata,
            request_id=f"phase83-{variant}-{session['session_id']}-t{turn}",
            session_id=f"phase83-{variant}-{session['session_id']}",
        )
    ...
    return {
        "kind": "phase83_real_api_multiturn_transcript",
...
def _generate(variant: str, clean: bool) -> int:
    ...
    if not freeze["passed"]:
        raise SystemExit(f"Phase83 generation freeze failed: {freeze}")
...
def _api_smoke(clean: bool) -> int:
    ...
    if clean:
        output_path.unlink(missing_ok=True)
    freeze = _freeze_check()
    secret = "SYNTHETIC_PHASE83_API_SMOKE_PRIVATE_DO_NOT_ECHO"
    old_base_model = os.environ.get("PFE_BASE_MODEL")
    os.environ["PFE_BASE_MODEL"] = str(MODEL_PATH)
    try:
        ...
        request = ChatCompletionRequest(
            ...
            request_id="phase83-api-smoke-request",
            session_id="phase83-api-smoke-session",
        )
        ...
        checks = {
            ...
            "persona_v2_contract_applied": contract.get("contract") == "contract_persona_guarded_v2"
...
def _collect_metrics() -> dict[str, dict[str, Any]]:
    return {
        ...
        for variant in PHASE83_VARIANTS
    }
...
def _public_private_audit() -> dict[str, Any]:
    ...
    expected = PHASE83_SESSION_COUNT * len(PHASE83_VARIANTS)
    checks = {
    ...
    return {
        "kind": "phase83_public_private_transcript_audit",
        "passed": all(checks.values()),
        "checks": checks,
...
def _ordinary_identity(transcripts: Mapping[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    base = by_variant.get(PHASE83_VARIANTS[0], {})
    runtime = by_variant.get(PHASE83_VARIANTS[1], {})
    details = []
    ...
    return {
        "kind": "phase83_ordinary_passthrough_identity",
...
def _evidence_manifest() -> dict[str, Any]:
    ...
    return {
        "kind": "phase83_evidence_manifest",
        "files": files,
...
def _finalize() -> int:
    ...
    decision = build_phase83_decision(
        metrics=metrics,
    ...
    comparison = {
        "kind": "phase83_persona_api_contract_v2_comparison",
        "created_at": _utcnow(),
        "model": "Qwen2.5-1.5B-Instruct",
        "model_revision": MODEL_REVISION,
        ...
        "phase82_canonical_reference": {
            "base_target_score": 0.5125,
            "runtime_target_score": 0.5675,
            "runtime_gain_vs_base": 0.055,
            "runtime_route_accuracy": 0.6,
            "runtime_truncation_rate": 0.2667,
            "runtime_ordinary_score": 0.92,
            "source": "docs/demo/phase82-mid-model-runtime-contract/phase82-final-decision.json",
    ...
    _write_json(EVIDENCE_ROOT / "phase83-final-decision.json", decision)
    _write_text(EVIDENCE_ROOT / "output_examples.md", _output_examples(transcripts))
    _write_text(EVIDENCE_ROOT / "phase83-final-decision.md", f"""# Phase83 Final Decision

Recommendation: **{decision['recommendation']}**

- Lifecycle status: `{decision['status']}`
- Base target score: `{decision['target_scores'][PHASE83_VARIANTS[0]]}`
- Persona V2 target score: `{decision['target_scores'][PHASE83_VARIANTS[1]]}`
...
Phase83 uses a fresh `simulated_usage` benchmark through the PFE chat pipeline. It contains no actual user feedback and authorizes no automatic deployment, promotion, Hermes attachment, or product-default change.
""")
    _write_text(EVIDENCE_ROOT / "phase83-runbook.md", """# Phase83 Runbook
...
""")
    next_goal = (
        "Build an opt-in manual trial pack for contract_persona_guarded_v2 and collect explicit human review without changing the product default."
        if decision["recommendation"] == "phase84_opt_in_manual_runtime_trial"
        else "Use Phase83 failure slices to repair runtime behavior or rewrite the adapter training objective before another claim."
    )
    _write_text(EVIDENCE_ROOT / "next-pursuit-goal.md", f"# Phase84 Pursuit Goal\n\n{next_goal}")
    ...
    _write_json(EVIDENCE_ROOT / "evidence_integrity.json", {
        "kind": "phase83_evidence_integrity",
        "passed": True,
    ...
    _write_json(EVIDENCE_ROOT / "finalization_state.json", {
        "kind": "phase83_finalization_state",
        "created_at": _utcnow(),
        "status": "finalized",
    })
    print(json.dumps({
...
def _full_regression() -> int:
    ...
    for command in commands:
        ...
        if result["exit_code"] != 0:
            break
    summary = {
        "kind": "phase83_full_regression_summary",
        "created_at": _utcnow(),
...
def _validate() -> int:
    ...
    decision = _read_json(EVIDENCE_ROOT / "phase83-final-decision.json")
    ...
    for path in EVIDENCE_ROOT.rglob("*"):
        ...
        try:
            ...
            continue
        if "SYNTHETIC_PHASE83_PRIVATE_" in text or "SYNTHETIC_PHASE83_API_SMOKE_PRIVATE" in text:
    ...
    summary = {
        "kind": "phase83_validation_summary",
        "created_at": _utcnow(),
        "passed": all(checks.values()),
        "checks": checks,
...
def main() -> int:
    ...
    generate.add_argument("--variant", choices=PHASE83_VARIANTS, required=True)
