#!/usr/bin/env python3
"""Run and persist the Phase63 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = _read_json(EVIDENCE_ROOT / "phase63-final-decision.json")
    comparison = _read_json(EVIDENCE_ROOT / "comparison_summary.json")
    phase63 = dict(comparison.get("phase63") or {})
    preflight_passed = phase63.get("typed_wire_preflight_status") == "passed"
    calibration_qualified = phase63.get("calibration_status") == "qualified"
    holdout_qualified = phase63.get("holdout_status") == "qualified"
    conflict_free = int(phase63.get("holdout_candidate_value_conflict_count") or 0) == 0
    expected = (
        ...
        else "hold_phase63_field_typed_candidate_wire"
    )
    preflight_outcomes = int(phase63.get("typed_wire_preflight_successful_model_output_count") or 0) + int(
        phase63.get("typed_wire_preflight_failed_judge_item_count") or 0
    )
    calibration_outcomes = int(phase63.get("calibration_successful_model_output_count") or 0) + int(
        phase63.get("calibration_failed_judge_item_count") or 0
    )
    holdout_outcomes = int(phase63.get("holdout_successful_model_output_count") or 0) + int(
        phase63.get("holdout_failed_judge_item_count") or 0
    )
    raw_rows = []
    for directory_name in ("evidence-typed-wire-preflight", "evidence-evaluator-calibration", "evidence-evaluator-holdout"):
        directory = EVIDENCE_ROOT / directory_name
        for alias in ("semantic_judge_alpha", "semantic_judge_beta"):
            raw_rows.extend(_read_jsonl(directory / f"typed_wire_failure_attempts_{alias}.jsonl"))
    ...
    passed = (
        ...
        and decision.get("runtime_replay_allowed_in_phase63") is False
    ...
    return {
        "name": "phase63_evidence_consistency",
        "command": ["internal", "phase63_evidence_consistency"],
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "pfe-core/pfe_core/phase63_field_typed_candidate_wire.py",
                "tools/phase63_prepare.py",
                "tools/phase63_execute.py",
                "tools/phase63_finalize_evidence.py",
                "tools/phase63_validate.py",
                "tests/test_phase63_field_typed_candidate_wire.py",
            ],
        ),
        ("phase63_focused_and_phase62_to_45_regression", [python, "-m", "pytest", "-q", *regression_tests]),
    ...
    summary = {
        "kind": "phase63_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase63 validation: {summary['status']}"]
    lines.extend(
