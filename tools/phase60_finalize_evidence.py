#!/usr/bin/env python3
"""Finalize Phase60 flat-schema compatibility evidence for every gated outcome."""
...
from pfe_core.phase60_flat_schema_compatibility import build_phase60_decision
...
def _stage_counts(directory: Path) -> dict[str, int]:
    successes = sum(len(_read_jsonl(directory / f"judge_flat_results_{alias}.jsonl")) for alias in ALIASES)
    failure_attempts = sum(
        len(_read_jsonl(directory / f"schema_failure_attempts_{alias}.jsonl")) for alias in ALIASES
    )
...
def _failure_rows(directory: Path) -> list[dict[str, Any]]:
    ...
    for alias in ALIASES:
        rows.extend(_read_jsonl(directory / f"schema_failure_attempts_{alias}.jsonl"))
    return rows
...
def _failure_analysis(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "kind": "phase60_flat_candidate_failure_analysis",
...
def _manifest() -> dict[str, Any]:
    ...
    return {"kind": "phase60_evidence_manifest", "file_count": len(files), "files": files, "manifest_sha256": digest}
...
def main() -> int:
    ...
    baseline = _read_json(EVIDENCE_ROOT / "evidence-baseline/phase59_canonical_snapshot.json")
    ...
    all_raw_failures = (
        _failure_rows(preflight_dir) + _failure_rows(calibration_dir) + _failure_rows(holdout_dir)
    )
    ...
    comparison = {
        "kind": "phase60_flat_schema_compatibility_comparison",
        "phase59": {
            "recommendation": baseline.get("phase59_recommendation"),
            "calibration_status": baseline.get("phase59_status"),
            "calibration_accuracy": baseline.get("phase59_accuracy"),
            "successful_model_output_count": baseline.get("phase59_successful_model_output_count"),
            "failed_judge_item_count": baseline.get("phase59_failed_judge_item_count"),
            "schema_failure_count": baseline.get("phase59_schema_failure_count"),
            "raw_invalid_responses_preserved": baseline.get("phase59_raw_invalid_responses_preserved"),
        },
        "phase60": {
            "schema_preflight_status": preflight.get("status") or "not_run",
            "schema_preflight_successful_model_output_count": preflight_counts["successful_model_output_count"],
            "schema_preflight_failed_judge_item_count": preflight_failed_items,
            "schema_preflight_raw_failure_attempt_count": preflight_counts["raw_failure_attempt_count"],
            ...
            "raw_schema_failures_preserved": raw_failures_preserved,
        },
    ...
    integrity_checks = {
        "phase59_canonical_snapshot_passed": baseline.get("passed") is True,
    ...
    integrity = {
        "kind": "phase60_evidence_integrity",
    ...
    if all_raw_failures:
        _write_json(
            EVIDENCE_ROOT / "protocol_compatibility_failure_evidence.json",
            {
                "kind": "phase60_protocol_compatibility_failure_evidence",
                "status": "preserved",
    ...
    _write_json(EVIDENCE_ROOT / "phase60-final-decision.json", decision)
    _write_json(EVIDENCE_ROOT / "evidence_integrity.json", integrity)
    _write_text(
        EVIDENCE_ROOT / "phase60-final-decision.md",
        f"""# Phase60 Final Decision

## 结论

最终 recommendation 为 **{decision['recommendation']}**。Flat-schema preflight 为 `{preflight.get('status')}`，calibration accuracy 为 `{calibration.get('accuracy')}`，holdout accuracy 为 `{holdout.get('accuracy')}`。
...
""",
    ...
    _write_text(
        EVIDENCE_ROOT / "phase60-runbook.md",
        """# Phase60 Runbook
...
```bash
.venv/bin/python tools/phase60_prepare.py --clean-evidence
.venv/bin/python tools/phase60_execute.py --stage preflight --ollama-endpoint http://127.0.0.1:11435 --timeout 900
.venv/bin/python tools/phase60_execute.py --stage calibration --ollama-endpoint http://127.0.0.1:11435 --timeout 900
.venv/bin/python tools/phase60_execute.py --stage holdout --ollama-endpoint http://127.0.0.1:11435 --timeout 900
.venv/bin/python tools/phase60_finalize_evidence.py
.venv/bin/python tools/phase60_validate.py
```

Do not change candidate generation, fixtures, prompts, schema, retry count, or gates after prepare. Stop after the first failed gate and finalize that path.
""",
    )
    if decision["phase61_external_replay_design_eligible"]:
        next_goal = (
            "Build Phase61 as a frozen historical replay of the manually reviewable Phase60 evaluator. Preserve the flat schema, "
            "candidate generation, composer, and gates. Require per-phase accuracy, zero false accepts, zero unsupported candidates, "
            "and no hard-rule conflict before considering a minimal runtime A/B. Do not train, attach Hermes, change defaults, "
            "auto-promote, or claim actual user benefit."
        )
    else:
        next_goal = (
            "Hold Phase60. Analyze only aggregate frozen preflight or candidate-selection failures, choose one structural correction, "
            "and freeze fresh fixtures before more model calls. Do not run runtime A/B, train, attach Hermes, change defaults, or relax gates."
        )
    ...
    finalization = {
        "kind": "phase60_finalization_state",
