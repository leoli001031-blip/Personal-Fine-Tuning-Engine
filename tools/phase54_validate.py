#!/usr/bin/env python3
"""Run and persist the Phase54 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = json.loads((EVIDENCE_ROOT / "phase54-final-decision.json").read_text(encoding="utf-8"))
    comparison = json.loads((EVIDENCE_ROOT / "comparison_summary.json").read_text(encoding="utf-8"))
    phase54 = dict(comparison.get("phase54") or {})
    holdout_qualified = phase54.get("holdout_status") == "qualified"
    expected = (
    ...
    holdout_calls_ok = (
        int(phase54.get("holdout_real_model_call_count") or 0) == 300
        if phase54.get("calibration_status") == "qualified"
        else int(phase54.get("holdout_real_model_call_count") or 0) == 0
    )
    passed = (
        integrity.get("passed") is True
        and decision.get("recommendation") == expected
        and int(phase54.get("calibration_real_model_call_count") or 0) == 240
        and holdout_calls_ok
        and phase54.get("final_label_generated_by_deterministic_composer") is True
        and int(phase54.get("holdout_judge_direct_label_count") or 0) == 0
    ...
    return {
        "name": "phase54_evidence_consistency",
        "command": ["internal", "phase54_evidence_consistency"],
        ...
        "output_tail": [
            ...
            f"calibration={phase54.get('calibration_accuracy')} holdout={phase54.get('holdout_accuracy')}"
        ],
    }
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "pfe-core/pfe_core/phase54_typed_proposition_evaluator.py",
                "tools/phase54_prepare.py",
                "tools/phase54_typed_evaluator.py",
                "tools/phase54_finalize_evidence.py",
                "tools/phase54_validate.py",
                "tests/test_phase54_typed_proposition_evaluator.py",
            ],
        ),
        (
            "phase54_focused_and_phase53_to_45_regression",
            [
                python,
                "-m",
                "pytest",
    ...
    summary = {
        "kind": "phase54_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase54 validation: {summary['status']}"]
    lines.extend(
