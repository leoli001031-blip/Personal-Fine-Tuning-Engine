#!/usr/bin/env python3
"""Run and persist the Phase56 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = json.loads((EVIDENCE_ROOT / "phase56-final-decision.json").read_text(encoding="utf-8"))
    comparison = json.loads((EVIDENCE_ROOT / "comparison_summary.json").read_text(encoding="utf-8"))
    phase56 = dict(comparison.get("phase56") or {})
    holdout_qualified = phase56.get("holdout_status") == "qualified"
    expected = (
    ...
    holdout_calls_ok = (
        int(phase56.get("holdout_real_model_call_count") or 0) == 300
        if phase56.get("calibration_status") == "qualified"
        else int(phase56.get("holdout_real_model_call_count") or 0) == 0
    )
    passed = (
        integrity.get("passed") is True
        and decision.get("recommendation") == expected
        and int(phase56.get("calibration_real_model_call_count") or 0) == 240
        and holdout_calls_ok
        and phase56.get("final_label_generated_by_deterministic_composer") is True
        and int(phase56.get("holdout_judge_direct_label_count") or 0) == 0
        and int(phase56.get("holdout_composer_received_ungrounded_atom_count") or 0) == 0
    ...
    return {
        "name": "phase56_evidence_consistency",
        "command": ["internal", "phase56_evidence_consistency"],
        ...
        "output_tail": [
            ...
            f"calibration={phase56.get('calibration_accuracy')} holdout={phase56.get('holdout_accuracy')}"
        ],
    }
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "tools/phase56_prepare.py",
                "tools/phase56_span_evaluator.py",
                "tools/phase56_finalize_evidence.py",
                "tools/phase56_validate.py",
                "tests/test_phase56_evidence_span_grounded_atomic.py",
            ],
        ),
        (
            "phase56_focused_and_phase55_to_45_regression",
            [
                python,
                "-m",
                "pytest",
    ...
    summary = {
        "kind": "phase56_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase56 validation: {summary['status']}"]
    lines.extend(
