#!/usr/bin/env python3
"""Generate real Qwen3-4B Phase49 provenance runtime ablation transcripts."""
...
def _freeze_check(sessions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    source_checks = {
        ...
        "phase46_generic_scorer_source": scorer.get("phase46_generic_scorer_source_sha256") == _sha256(PHASE46_SOURCE),
        "phase48_compact_v1_source": scorer.get("phase48_compact_v1_source_sha256") == _sha256(PHASE48_SOURCE),
        "phase49_source": scorer.get("phase49_source_sha256") == _sha256(PHASE49_SOURCE),
    }
    scorer_ok = (
        all(source_checks.values())
        and scorer.get("calibration_status") == "passed"
        and float(scorer.get("calibration_exact_label_accuracy") or 0.0) == 1.0
    )
    ...
    return {
        "kind": "phase49_generation_freeze_check",
...
def _run_session(
    *,
    ...
    transcript: dict[str, Any] = {
        "kind": "phase49_real_provenance_runtime_ablation_transcript",
...
def main() -> int:
    ...
    if freeze["passed"] is not True:
        raise SystemExit(f"Phase49 scorer/holdout/protocol freeze failed: {freeze}")
    protocol = _read_json(PROTOCOL_PATH)
    ...
    report = aggregate_phase49_variant(transcripts, sessions, reference_targets=reference_targets)
    persistence = [
