#!/usr/bin/env python3
"""Finalize Phase62 risk-asymmetric consensus evidence."""
...
from pfe_core.phase62_risk_asymmetric_candidate_consensus import build_phase62_decision
...
def _failure_analysis(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "kind": "phase62_consensus_failure_analysis",
...
def _manifest() -> dict[str, Any]:
    ...
    return {"kind": "phase62_evidence_manifest", "file_count": len(files), "files": files, "manifest_sha256": digest}
...
def main() -> int:
    ...
    baseline = _read_json(EVIDENCE_ROOT / "evidence-baseline/phase61_canonical_snapshot.json")
    ...
    comparison = {
        "kind": "phase62_risk_asymmetric_consensus_comparison",
        "phase61": {
            "recommendation": baseline.get("phase61_recommendation"),
            "preflight_status": baseline.get("phase61_preflight_status"),
            "calibration_status": baseline.get("phase61_calibration_status"),
            "calibration_accuracy": baseline.get("phase61_calibration_accuracy"),
            "typed_exact_match_rate": baseline.get("phase61_typed_exact_match_rate"),
            "candidate_selection_exact_match_rate": baseline.get("phase61_candidate_selection_exact_match_rate"),
        ...
        "phase62": {
            "protocol_preflight_status": preflight.get("status") or "not_run",
            "protocol_preflight_successful_model_output_count": preflight_counts["successful_model_output_count"],
            "protocol_preflight_failed_judge_item_count": preflight_failed_items,
    ...
    integrity_checks = {
        "phase61_canonical_snapshot_passed": baseline.get("passed") is True,
    ...
    integrity = {
        "kind": "phase62_evidence_integrity",
    ...
    if all_raw_failures:
        _write_json(
            ...
            {
                "kind": "phase62_wire_protocol_failure_evidence",
                "status": "preserved",
    ...
    _write_json(EVIDENCE_ROOT / "phase62-final-decision.json", decision)
    _write_json(EVIDENCE_ROOT / "evidence_integrity.json", integrity)
    _write_text(
        EVIDENCE_ROOT / "phase62-final-decision.md",
        f"""# Phase62 Final Decision

## 结论

最终 recommendation 为 **{decision['recommendation']}**。Protocol preflight 为 `{preflight.get('status')}`，calibration accuracy 为 `{calibration.get('accuracy')}`，holdout accuracy 为 `{holdout.get('accuracy')}`。
...
""",
    ...
    _write_text(
        EVIDENCE_ROOT / "phase62-runbook.md",
        """# Phase62 Runbook
...
""",
    ...
    if decision["phase63_external_replay_design_eligible"]:
        next_goal = (
            "Build Phase63 as a frozen historical replay of the manually reviewable Phase62 consensus evaluator. Preserve compact "
            "wire, risk-asymmetric consensus, candidate generation, composer, and gates. Require per-phase accuracy, zero false "
            "accepts, zero unsupported candidates, and zero candidate-value conflicts before considering a minimal runtime A/B. "
            "Do not train, attach Hermes, change defaults, auto-promote, or claim actual user benefit."
        )
    else:
        next_goal = (
            "Hold Phase62. Analyze only aggregate frozen consensus failures, choose one structural correction, and freeze fresh fixtures "
            "before more model calls. Do not run runtime A/B, train, attach Hermes, change defaults, or relax gates."
        )
    ...
    finalization = {
        "kind": "phase62_finalization_state",
