#!/usr/bin/env python3
"""Freeze Phase59 proposition-candidate calibration and holdout before calls."""
...
PHASE58_ROOT = REPO_ROOT / "docs/demo/phase58-clause-addressed-grounding"
...
def _phase58_snapshot() -> dict[str, Any]:
    decision = _read_json(PHASE58_ROOT / "phase58-final-decision.json")
    report = _read_json(PHASE58_ROOT / "evidence-evaluator-calibration/clause_evaluator_report.json")
    failures = _read_json(PHASE58_ROOT / "evidence-evaluator-calibration/failure_analysis.json")
    integrity = _read_json(PHASE58_ROOT / "evidence_integrity.json")
    manifest = _read_json(PHASE58_ROOT / "evidence_manifest.json")
    manifest_ok = _verify_manifest(manifest)
    passed = (
        decision.get("recommendation") == "hold_phase58_clause_addressed_grounding"
        and decision.get("phase59_minimal_runtime_ab_design_eligible") is False
        and report.get("status") == "not_qualified"
        and integrity.get("passed") is True
        and manifest_ok
    )
    return {
        "kind": "phase59_phase58_canonical_snapshot",
        "passed": passed,
        "phase58_recommendation": decision.get("recommendation"),
        "phase58_status": report.get("status"),
        "phase58_accuracy": report.get("accuracy"),
        "phase58_typed_exact_match_rate": report.get("typed_exact_match_rate"),
        "phase58_grounding_validity_rate": report.get("grounding_validity_rate"),
        "phase58_invalid_atom_count": report.get("invalid_atom_count"),
        "phase58_invalid_dangerous_atom_count": report.get("invalid_dangerous_atom_count"),
        "phase58_label_failure_count": failures.get("label_failure_count"),
        "phase58_manifest_sha256": manifest.get("manifest_sha256"),
        "phase58_manifest_verified": manifest_ok,
        "phase58_source_modified": False,
    }
    ...
    for split in ("calibration", "holdout"):
        ...
        cases.extend(
            ...
            for row in _read_json(
                PHASE58_ROOT / f"evidence-evaluator-{split}/{filename}"
            ).get("cases") or []
        )
    cases.extend(
        _read_jsonl(
            REPO_ROOT / "docs/demo/phase57-span-evaluator-historical-replay/evidence-historical-replay/blind_items_public.jsonl"
        )
    )
    return cases
...
def main() -> int:
    ...
    split = build_phase59_split_integrity(
        calibration["cases"], holdout["cases"], historical_cases=historical
    )
    calibration_blind = build_phase59_blind_items(
        calibration["cases"], seed=5901, prefix="phase59-calibration-blind"
    )
    holdout_blind = build_phase59_blind_items(
        holdout["cases"], seed=5902, prefix="phase59-holdout-blind"
    )
    ...
    schema_template = phase59_ollama_json_schema(
        [
            {"candidate_id": "p001", "field": "source_registration"},
            {"candidate_id": "p002", "field": "user_outcome_status"},
            {"candidate_id": "p003", "field": "test_to_user_outcome_relation"},
        ]
    )
    protocol = {
        "kind": "phase59_frozen_proposition_candidate_protocol",
        "structural_change": "pre_ground_field_value_clause_candidates_then_select_candidate_id",
        "phase58_aggregate_failure_basis": {
            ...
            "label_failure_count": baseline.get("phase58_label_failure_count"),
            "failure_classes": ["unsupported_value_clause_binding", "cross_field_fixture_ambiguity"],
            "individual_phase58_rows_used_for_prompt_tuning": False,
        },
        ...
        "judges_receive_gold_candidate_ids": False,
    ...
    freeze = {
        "kind": "phase59_pre_model_call_freeze",
        "phase58_canonical_snapshot_passed": baseline["passed"],
        ...
        "phase59_evaluator_source_sha256": _sha256(PHASE59_SOURCE),
        "phase59_selection_rubric_sha256": stable_hash(PHASE59_SELECTION_RUBRIC),
        "phase59_schema_template_sha256": stable_hash(schema_template),
    ...
    source_manifest = {
        "kind": "phase59_source_boundary_manifest",
        ...
        "phase58_rows_used_for_training": False,
        "phase58_individual_rows_used_for_tuning": False,
    }
    preparation = {
        "kind": "phase59_preparation_decision",
    ...
    runtime = {
        "kind": "phase59_runtime_status",
        "runtime_replay_status": "not_requested_in_phase59",
        "runtime_replay_model_call_count": 0,
        "runtime_prompt_changed": False,
        "router_changed": False,
    }
    training = {
        "kind": "phase59_training_attempt",
    ...
    _write_json(EVIDENCE_ROOT / "evidence-baseline/phase58_canonical_snapshot.json", baseline)
