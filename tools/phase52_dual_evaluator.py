#!/usr/bin/env python3
"""Run the frozen Phase52 Gemma/Qwen evaluator protocol."""
...
def _judge_one(
    *, item: Mapping[str, Any], alias: str, model: str, endpoint: str, timeout: int
    ...
    prompt = build_phase52_semantic_judge_prompt(item)
    payload = {
        "model": model,
        "stream": False,
        "think": False,
...
def _fixture_paths(split: str) -> tuple[Path, Path, Path]:
    directory_name = {
        "calibration": "evidence-evaluator-calibration",
        "holdout": "evidence-evaluator-holdout",
        "replay": "evidence-phase51-runtime-replay",
    }[split]
...
def _freeze_check(split: str, public: list[dict[str, Any]], hidden: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    source_ok = (
        freeze.get("phase51_evaluator_source_sha256") == _sha256(PHASE51_SOURCE)
        and freeze.get("phase52_evaluator_source_sha256") == _sha256(PHASE52_SOURCE)
    )
    protocol_ok = (
    ...
    if split == "calibration":
        ...
        phase51_public = _read_jsonl(
            PHASE51_ROOT / "evidence-runtime-dual-eval" / "blind_items_public.jsonl"
        )
        phase51_hidden = _read_json(
            PHASE51_ROOT / "evidence-runtime-dual-eval" / "blind_hidden_key.json"
        ).get("items") or []
        data_ok = (
            stable_hash(phase51_public) == freeze.get("phase51_replay_source_public_sha256")
            and stable_hash(phase51_hidden) == freeze.get("phase51_replay_source_hidden_sha256")
            and stable_hash(public) == freeze.get("phase52_replay_public_sha256")
            and stable_hash(hidden) == freeze.get("phase52_replay_hidden_sha256")
        )
        prerequisite_ok = (
            _read_json(EVIDENCE_ROOT / "evidence-evaluator-calibration" / "dual_evaluator_report.json").get("status")
            == "qualified"
            and _read_json(EVIDENCE_ROOT / "evidence-evaluator-holdout" / "dual_evaluator_report.json").get("status")
            == "qualified"
        )
    return {
        "kind": "phase52_dual_evaluator_freeze_check",
        "split": split,
...
def main() -> int:
    ...
    parser.add_argument("--split", choices=("calibration", "holdout", "replay"), required=True)
    ...
    if freeze_check["passed"] is not True:
        raise SystemExit(f"Phase52 {args.split} freeze check failed: {freeze_check}")
    ...
    availability = {
        "kind": "phase52_judge_model_availability",
        "split": args.split,
    ...
    if availability["passed"] is not True:
        raise SystemExit(f"Phase52 judge models unavailable: {missing}")
        ...
        for index, item in enumerate(selected, start=1):
            ...
            for attempt in range(1, 3):
                try:
                    ...
                    print(f"[{args.split}:{alias}] {index}/{len(selected)} {item_id} {result['label']}", flush=True)
                    error = None
                    break
    ...
    if args.split in {"calibration", "holdout"}:
        report = evaluate_phase52_dual_evaluator(
            split=args.split,
            public_items=public,
            hidden_key=hidden,
        ...
        report = summarize_phase52_replay(
            public_items=public,
            hidden_key=hidden,
            judge_results=all_results,
            judge_aliases=aliases,
    ...
    qualified = report.get("status") == ("completed" if args.split == "replay" else "qualified")
