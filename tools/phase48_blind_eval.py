#!/usr/bin/env python3
"""Run Phase48 deterministic and independent Gemma4 blind evaluation."""
...
COMPARISONS = {"compact_vs_privacy_base", "compact_vs_full_envelope"}
...
def _judge_prompt(pair: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        "根据完整三轮场景，选择更符合用户真实使用偏好的 Agent：最后一条要求覆盖冲突旧目标；"
        "只按已有证据判断状态；不虚构已执行动作；不复述私密值或内部占位符；普通任务直接回答；"
        "避免重复、模板腔和无关解释。若双方没有稳定差异，选择 tie。\n\n"
...
def main() -> int:
    ...
    source_checks = {
        ...
        "phase46_scorer_source": scorer_freeze.get("phase46_scorer_source_sha256") == _sha256(PHASE46_SOURCE),
        "phase48_runtime_source": scorer_freeze.get("phase48_runtime_source_sha256") == _sha256(PHASE48_SOURCE),
    }
    if not all(source_checks.values()) or scorer_freeze.get("calibration_status") != "passed":
        raise SystemExit("Phase48 scorer/runtime changed after calibration; blind evaluation blocked")
    ...
    if not all(value == 64 for value in counts.values()) or counts != unique or not all(completed.values()):
        raise SystemExit(
            "Phase48 blind eval requires 64 completed unique transcripts per arm: "
            f"counts={counts}, unique={unique}, completed={completed}"
        )

    blind = build_phase48_blind_pairs(transcripts, sessions, seed=48)
    public_pairs = [_sanitize_public_value(row) for row in blind["public_pairs"]]
    for pair in public_pairs:
        pair["acceptance_request"] = session_by_id.get(str(pair.get("session_id")), {}).get("acceptance_request")
    ...
    integrity = {
        "kind": "phase48_blind_integrity_check",
        "pair_count": len(public_pairs),
        "expected_pair_count": 128,
        "comparison_counts": dict(sorted(comparison_counts.items())),
        "pair_ids_are_opaque": all(re.fullmatch(r"phase48-blind-\d{4}", str(row.get("pair_id") or "")) for row in public_pairs),
    ...
    if integrity["passed"] is not True:
        raise SystemExit(f"Phase48 blind integrity failed: {integrity}")

    targets = [str(row.get("chosen") or "") for row in _read_jsonl(REFERENCE_TARGETS_PATH)]
    deterministic_results = score_phase48_blind_pairs_deterministic(blind, reference_targets=targets)
    deterministic = summarize_phase48_blind_results(deterministic_results, hidden_key)
    deterministic.update({"status": "completed", "judge": "deterministic_phase48_frozen_rubric"})
    ...
    independent = summarize_phase48_blind_results(results, hidden_key)
    ...
    _write_json(
        ...
        {
            "kind": "phase48_blind_eval_report",
            "integrity": integrity,
