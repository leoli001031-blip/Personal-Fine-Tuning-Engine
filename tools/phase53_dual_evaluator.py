#!/usr/bin/env python3
"""Run the frozen Phase53 Gemma/Qwen scope-recovery protocol."""
...
def _judge_one(
    *, item: Mapping[str, Any], alias: str, model: str, endpoint: str, timeout: int
    ...
    prompt = build_phase53_semantic_judge_prompt(item)
    payload = {
        "model": model,
        "stream": False,
        "think": False,
...
def _freeze_check(split: str, public: list[dict[str, Any]], hidden: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    source_ok = (
        freeze.get("phase52_evaluator_source_sha256") == _sha256(PHASE52_SOURCE)
        and freeze.get("phase53_evaluator_source_sha256") == _sha256(PHASE53_SOURCE)
    )
    protocol_ok = (
    ...
    return {
        "kind": "phase53_dual_evaluator_freeze_check",
        "split": split,
...
def main() -> int:
    ...
    if freeze_check["passed"] is not True:
        raise SystemExit(f"Phase53 {args.split} freeze check failed: {freeze_check}")
    ...
    availability = {
        "kind": "phase53_judge_model_availability",
        "split": args.split,
    ...
    if availability["passed"] is not True:
        raise SystemExit(f"Phase53 judge models unavailable: {missing}")
        ...
        for index, item in enumerate(public, start=1):
            ...
            for attempt in range(1, 3):
                try:
                    ...
                    print(f"[{args.split}:{alias}] {index}/{len(public)} {item_id} {result['label']}", flush=True)
                    error = None
                    break
