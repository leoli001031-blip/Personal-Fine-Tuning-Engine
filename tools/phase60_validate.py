#!/usr/bin/env python3
"""Run and persist the Phase60 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = _read_json(EVIDENCE_ROOT / "phase60-final-decision.json")
    comparison = _read_json(EVIDENCE_ROOT / "comparison_summary.json")
    phase60 = dict(comparison.get("phase60") or {})
    preflight_passed = phase60.get("schema_preflight_status") == "passed"
    calibration_qualified = phase60.get("calibration_status") == "qualified"
    holdout_qualified = phase60.get("holdout_status") == "qualified"
    expected = (
        "recommend_phase60_flat_candidate_evaluator_for_manual_review_only"
        if preflight_passed and calibration_qualified and holdout_qualified
        else "hold_phase60_flat_schema_compatibility_recovery"
    )
    preflight_outcomes = int(phase60.get("schema_preflight_successful_model_output_count") or 0) + int(
        phase60.get("schema_preflight_failed_judge_item_count") or 0
    )
    calibration_outcomes = int(phase60.get("calibration_successful_model_output_count") or 0) + int(
        phase60.get("calibration_failed_judge_item_count") or 0
    )
    holdout_outcomes = int(phase60.get("holdout_successful_model_output_count") or 0) + int(
        phase60.get("holdout_failed_judge_item_count") or 0
    )
    raw_rows = []
    for directory_name in ("evidence-schema-preflight", "evidence-evaluator-calibration", "evidence-evaluator-holdout"):
        directory = EVIDENCE_ROOT / directory_name
        for alias in ("semantic_judge_alpha", "semantic_judge_beta"):
            raw_rows.extend(_read_jsonl(directory / f"schema_failure_attempts_{alias}.jsonl"))
    ...
    passed = (
        ...
        and decision.get("runtime_replay_allowed_in_phase60") is False
    ...
    return {
        "name": "phase60_evidence_consistency",
        "command": ["internal", "phase60_evidence_consistency"],
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "pfe-core/pfe_core/phase60_flat_schema_compatibility.py",
                "tools/phase60_prepare.py",
                "tools/phase60_execute.py",
                "tools/phase60_finalize_evidence.py",
                "tools/phase60_validate.py",
                "tests/test_phase60_flat_schema_compatibility.py",
            ],
        ),
        ("phase60_focused_and_phase59_to_45_regression", [python, "-m", "pytest", "-q", *regression_tests]),
    ...
    summary = {
        "kind": "phase60_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase60 validation: {summary['status']}"]
    lines.extend(
