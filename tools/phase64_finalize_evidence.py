#!/usr/bin/env python3
"""Finalize Phase64 historical replay evidence without runtime changes."""
...
from pfe_core.phase64_field_typed_historical_replay import build_phase64_decision
...
def _failure_analysis(report: Mapping[str, Any]) -> dict[str, Any]:
    label_failures = [
        dict(row) for row in report.get("details") or [] if row.get("passed") is False
    ]
            ...
            conflict_rows.append(
                {
                    ...
                    "field_consensus": row.get("field_consensus"),
                }
            )
    return {
        "kind": "phase64_historical_replay_failure_analysis",
        ...
        "conflict_item_count": len(conflict_rows),
        "label_transition_counts": dict(sorted(transition_counts.items())),
        "wire_consensus_composer_or_fixture_modified_after_replay": False,
...
def _manifest() -> dict[str, Any]:
    ...
    return {
        "kind": "phase64_evidence_manifest",
...
def main() -> int:
    ...
    baseline = _read_json(EVIDENCE_ROOT / "evidence-baseline/phase63_canonical_snapshot.json")
    ...
    failed_judge_items = int(report.get("failure_count") or 0)
    raw_failure_attempts = int(report.get("raw_failure_attempt_count") or 0)
    judge_item_outcomes = calls + failed_judge_items
    actual_judge_attempts = calls + raw_failure_attempts
    phase63_holdout = _read_json(
        PHASE63_ROOT / "evidence-evaluator-holdout/candidate_evaluator_report.json"
    )
    comparison = {
        "kind": "phase64_historical_replay_comparison",
        "phase63": {
            "recommendation": _read_json(
                PHASE63_ROOT / "phase63-final-decision.json"
            ).get("recommendation"),
            "holdout_accuracy": phase63_holdout.get("accuracy"),
            "holdout_typed_exact_match_rate": phase63_holdout.get("typed_exact_match_rate"),
            "holdout_candidate_selection_exact_match_rate": phase63_holdout.get(
                "candidate_selection_exact_match_rate"
            ),
            "holdout_raw_judge_typed_exact_match_rate": phase63_holdout.get(
        ...
        "phase64": {
            ...
            "successful_model_output_count": calls,
            "failed_judge_item_count": failed_judge_items,
            "judge_item_outcome_count": judge_item_outcomes,
            "actual_judge_attempt_count": actual_judge_attempts,
    ...
    integrity_checks = {
        "phase63_canonical_snapshot_passed": baseline.get("passed") is True,
        "historical_replay_integrity_passed": replay_integrity.get("passed") is True,
        "freeze_check_passed": _read_json(replay_dir / "freeze_check.json").get("passed")
        is True,
        "replay_complete": (
            judge_item_outcomes == 1116
            and int(report.get("item_count") or 0) == 558
            and int(report.get("completed_item_count") or 0) + failed_judge_items == 558
            and int(report.get("judge_item_outcome_count") or 0) == 1116
            and actual_judge_attempts >= 1116
        ),
        "decision_matches_frozen_gates": decision["recommendation"]
        == expected_recommendation,
        "phase63_evaluator_unchanged": report.get("phase63_field_typed_wire_unchanged")
        is True,
    ...
    integrity = {
        "kind": "phase64_evidence_integrity",
    ...
    _write_json(EVIDENCE_ROOT / "phase64-final-decision.json", decision)
    _write_json(EVIDENCE_ROOT / "evidence_integrity.json", integrity)
    _write_text(
        EVIDENCE_ROOT / "phase64-final-decision.md",
        f"""# Phase64 Final Decision

## 结论

最终 recommendation 为 **{decision['recommendation']}**。Phase51-55 历史 holdout 总准确率为 `{report.get('accuracy')}`，各阶段结果为 `{report.get('per_phase')}`，false accepts 为 `{report.get('false_accept_count_on_reject_cases')}`，candidate conflicts 为 `{report.get('candidate_value_conflict_count')}`。
...
""",
    ...
    _write_text(
        EVIDENCE_ROOT / "phase64-runbook.md",
        """# Phase64 Runbook
...
```bash
.venv/bin/python tools/phase64_prepare.py --clean-evidence
.venv/bin/python tools/phase64_historical_replay.py --ollama-endpoint http://127.0.0.1:11435 --timeout 900
.venv/bin/python tools/phase64_finalize_evidence.py
.venv/bin/python tools/phase64_validate.py
```

The historical replay is a one-shot qualification of the frozen Phase63 evaluator. Results may be analyzed and archived, but never used to tune Phase63 inside Phase64.
""",
    )
    next_goal = (
        "Build Phase65 as one minimal shadow-only runtime-contract A/B using the externally qualified Phase63 evaluator. Freeze a representative simulated-usage set before calls; compare the current base/runtime path against exactly one boundary-contract candidate with real model outputs; require no safety, citation, structure, provenance, or user-outcome regression and at least one measurable boundary improvement. Do not train, attach Hermes, change the product default, auto-promote, or claim actual user benefit."
        if decision["phase65_minimal_runtime_ab_design_eligible"]
        else
        "Build Phase65 from the sealed aggregate Phase64 failure classes. Do not tune on individual replay rows. Introduce at most one structural evaluator change justified by the aggregate evidence, then freeze completely new calibration and holdout before any model calls. Do not run runtime A/B, train, attach Hermes, or change the product default."
    )
    ...
    finalization = {
        "kind": "phase64_finalization_state",
