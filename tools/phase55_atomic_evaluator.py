#!/usr/bin/env python3
"""Run the frozen Phase55 Gemma/Qwen atomic boundary protocol."""
...
def _judge_one(
    *, item: Mapping[str, Any], alias: str, model: str, endpoint: str, timeout: int, protocol: Mapping[str, Any]
    ...
    prompt = build_phase55_typed_judge_prompt(item)
    ...
    typed = validate_phase55_typed_extraction(parsed)
    return {
...
def _freeze_check(split: str, public: list[dict[str, Any]], hidden: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "kind": "phase55_atomic_evaluator_freeze_check",
        "split": split,
...
def main() -> int:
    ...
    if freeze_check["passed"] is not True:
        raise SystemExit(f"Phase55 {args.split} freeze check failed: {freeze_check}")
    ...
    availability = {
        "kind": "phase55_judge_model_availability",
        "split": args.split,
    ...
    if availability["passed"] is not True:
        raise SystemExit(f"Phase55 judge models unavailable: {missing}")
    ...
    for alias in aliases:
        ...
        for row in existing:
            try:
                validate_phase55_typed_extraction(row)
            except ValueError:
                continue
