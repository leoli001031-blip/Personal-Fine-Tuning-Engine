#!/usr/bin/env python3
"""Run and persist the Phase57 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = json.loads((EVIDENCE_ROOT / "phase57-final-decision.json").read_text(encoding="utf-8"))
    comparison = json.loads((EVIDENCE_ROOT / "comparison_summary.json").read_text(encoding="utf-8"))
    phase57 = dict(comparison.get("phase57") or {})
    replay_qualified = phase57.get("status") == "qualified"
    expected = (
    ...
    passed = (
        ...
        and int(phase57.get("real_model_call_count") or 0) == 1116
        and phase57.get("phase56_evaluator_unchanged") is True
    ...
    return {
        "name": "phase57_evidence_consistency",
        "command": ["internal", "phase57_evidence_consistency"],
        ...
        "output_tail": [
            ...
            f"replay={phase57.get('historical_replay_accuracy')} per_phase={phase57.get('per_phase')}"
        ],
    }
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "pfe-core/pfe_core/phase57_span_evaluator_historical_replay.py",
                "pfe-core/pfe_core/phase56_evidence_span_grounded_atomic.py",
                "tools/phase57_prepare.py",
                "tools/phase57_historical_replay.py",
                "tools/phase57_finalize_evidence.py",
                "tools/phase57_validate.py",
                "tests/test_phase57_span_evaluator_historical_replay.py",
            ],
        ),
        (
            "phase57_focused_and_phase56_to_45_regression",
            [
                python,
                "-m",
                "pytest",
    ...
    summary = {
        "kind": "phase57_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase57 validation: {summary['status']}"]
    lines.extend(
