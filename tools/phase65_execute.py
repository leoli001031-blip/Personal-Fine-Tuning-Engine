#!/usr/bin/env python3
"""Run frozen Phase65 scope-aware candidate calls and evaluation."""
...
PHASE65_EXECUTOR = TOOLS_ROOT / "phase65_execute.py"
...
def _freeze_check(
    ...
    source_ok = (
        ...
        and freeze.get("phase65_executor_source_sha256") == _sha256(PHASE65_EXECUTOR)
    )
    protocol_ok = (
    ...
    if stage == "preflight":
        ...
        if stage == "calibration":
            ...
            prerequisite_ok = (
                _read_json(
                    EVIDENCE_ROOT
                    / "evidence-evaluator-calibration/candidate_evaluator_report.json"
                ).get("status")
                == "qualified"
            )
    return {
        "kind": "phase65_scope_aware_freeze_check",
        "stage": stage,
...
def main() -> int:
    ...
    if freeze_check["passed"] is not True:
        raise SystemExit(f"Phase65 {args.stage} freeze check failed: {freeze_check}")

    protocol = _read_json(EVIDENCE_ROOT / "evaluator_protocol.json")
    workers = int(protocol.get("parallel_worker_count") or 1)
    if args.workers is not None and args.workers != workers:
        raise SystemExit(f"Phase65 worker count is frozen at {workers}; received --workers={args.workers}")
    ...
    availability = {
        "kind": "phase65_judge_model_availability",
        "stage": args.stage,
    ...
    if availability["passed"] is not True:
        raise SystemExit(f"Phase65 judge models unavailable: {missing}")
    ...
    for alias in aliases:
        ...
        if (output_path.exists() or failure_path.exists()) and not args.resume:
            raise SystemExit(
                f"Phase65 {args.stage} is frozen one-shot evidence; use --resume only after interruption"
            )
    ...
    if args.stage == "preflight":
        report = {
            "kind": "phase65_typed_wire_preflight_report",
            "stage": args.stage,
            "status": "passed"
        ...
        report.update(
            {
                "kind": "phase65_scope_aware_candidate_report",
                "successful_model_output_count": len(all_results),
