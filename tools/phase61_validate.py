#!/usr/bin/env python3
"""Run and persist the Phase61 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = _read_json(EVIDENCE_ROOT / "phase61-final-decision.json")
    comparison = _read_json(EVIDENCE_ROOT / "comparison_summary.json")
    phase61 = dict(comparison.get("phase61") or {})
    preflight_passed = phase61.get("wire_preflight_status") == "passed"
    calibration_qualified = phase61.get("calibration_status") == "qualified"
    holdout_qualified = phase61.get("holdout_status") == "qualified"
    expected = (
        ...
        else "hold_phase61_compact_candidate_wire_protocol"
    )
    preflight_outcomes = int(phase61.get("wire_preflight_successful_model_output_count") or 0) + int(
        phase61.get("wire_preflight_failed_judge_item_count") or 0
    )
    calibration_outcomes = int(phase61.get("calibration_successful_model_output_count") or 0) + int(
        phase61.get("calibration_failed_judge_item_count") or 0
    )
    holdout_outcomes = int(phase61.get("holdout_successful_model_output_count") or 0) + int(
        phase61.get("holdout_failed_judge_item_count") or 0
    )
    raw_rows = []
    for directory_name in ("evidence-wire-preflight", "evidence-evaluator-calibration", "evidence-evaluator-holdout"):
    ...
    passed = (
        ...
        and decision.get("runtime_replay_allowed_in_phase61") is False
    ...
    return {
        "name": "phase61_evidence_consistency",
        "command": ["internal", "phase61_evidence_consistency"],
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "pfe-core/pfe_core/phase61_compact_candidate_wire_protocol.py",
                "tools/phase61_prepare.py",
                "tools/phase61_execute.py",
                "tools/phase61_finalize_evidence.py",
                "tools/phase61_validate.py",
                "tests/test_phase61_compact_candidate_wire_protocol.py",
            ],
        ),
        ("phase61_focused_and_phase60_to_45_regression", [python, "-m", "pytest", "-q", *regression_tests]),
    ...
    summary = {
        "kind": "phase61_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase61 validation: {summary['status']}"]
    lines.extend(
