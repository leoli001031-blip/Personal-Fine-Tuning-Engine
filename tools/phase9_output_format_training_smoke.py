#!/usr/bin/env python3
"""Run the Phase9 output-format stability training smoke."""
...
def _allocate_workdir(args: argparse.Namespace) -> tuple[Path, bool]:
    ...
    return Path(tempfile.mkdtemp(prefix="pfe-phase9-output-format-")), not args.keep_workdir
...
def _maybe_run_qwen_training(args: argparse.Namespace, workdir: Path, preflight: Mapping[str, Any]) -> dict[str, Any]:
    logs_dir = workdir / "phase9-command-logs"
    ...
    if not preflight.get("ready_for_real_training"):
        return {
            ...
            "mock_fallback": False,
            "skip_reason": "phase9_qwen_mlx_preflight_blocked",
...
def _maybe_run_real_eval(args: argparse.Namespace, workdir: Path, training: Mapping[str, Any]) -> dict[str, Any]:
    ...
    store = Phase9OutputFormatTrainingStore(home=workdir / ".pfe", workspace=args.workspace)
...
def _copy_evidence(args: argparse.Namespace, payload: Mapping[str, Any]) -> dict[str, str]:
    ...
    output_path = evidence_dir / "phase9-smoke-output.json"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run Phase9: public contract sources -> short output-boundary samples -> "
            "Qwen3-0.6B MLX training attempt -> holdout eval decision."
        )
    )
    parser.add_argument("--repo-root", type=Path, default=repo_root)
    parser.add_argument("--python", default=_default_python(repo_root))
    parser.add_argument("--workspace", default="phase9_output_format_training")
    ...
    parser.add_argument("--model-id", default=PHASE9_RECOMMENDED_MODEL)
    ...
    parser.add_argument(
        "--phase8-baseline-eval-path",
        type=Path,
        default=repo_root / "docs/demo/phase8-signal-quality-training/evidence-real-qwen3-0.6b/eval_report.json",
    )
...
def main() -> int:
    ...
    try:
        ...
        finalized = finalize_phase9_output_format_trial(
            ...
            phase8_baseline_eval_path=args.phase8_baseline_eval_path,
        )
        payload = {
            "ok": True,
            "workspace": args.workspace,
