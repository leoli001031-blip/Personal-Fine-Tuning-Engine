#!/usr/bin/env python3
"""Run the Phase7 real signal-driven candidate training smoke."""
...
def _allocate_workdir(args: argparse.Namespace) -> tuple[Path, bool]:
    ...
    return Path(tempfile.mkdtemp(prefix="pfe-phase7-real-signal-")), not args.keep_workdir
...
def _maybe_run_qwen_training(args: argparse.Namespace, workdir: Path, preflight: Mapping[str, Any]) -> dict[str, Any]:
    logs_dir = workdir / "phase7-command-logs"
    ...
    if not preflight.get("ready_for_real_training"):
        return {
            ...
            "mock_fallback": False,
            "skip_reason": "phase7_qwen36_mlx_preflight_blocked",
...
def _maybe_run_real_eval(args: argparse.Namespace, workdir: Path, training: Mapping[str, Any]) -> dict[str, Any]:
    ...
    store = Phase7RealSignalTrainingStore(home=workdir / ".pfe", workspace=args.workspace)
...
def _copy_evidence(args: argparse.Namespace, payload: Mapping[str, Any]) -> dict[str, str]:
    ...
    output_path = evidence_dir / "phase7-smoke-output.json"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run Phase7: public contract sources -> simulated interaction signals -> "
            "signal-gated candidate samples -> Qwen3.6/MLX training attempt -> eval decision."
        )
    )
    parser.add_argument("--repo-root", type=Path, default=repo_root)
    parser.add_argument("--python", default=_default_python(repo_root))
    parser.add_argument("--workspace", default="phase7_real_signal_training")
    ...
    parser.add_argument("--model-id", default=PHASE7_RECOMMENDED_MODEL)
    ...
    parser.add_argument("--eval-max-tokens", type=int, default=180)
