#!/usr/bin/env python3
"""Prepare and freeze Phase55 atomic-evaluator evidence before model calls."""
...
from pfe_core.phase55_atomic_boundary_composition import (
    PHASE55_CALIBRATION_ACCURACY_GATE,
    PHASE55_CATEGORIES,
    PHASE55_EXTRACTION_RUBRIC,
    PHASE55_HOLDOUT_ACCURACY_GATE,
    PHASE55_PER_CATEGORY_ACCURACY_GATE,
    PHASE55_PER_FIELD_ACCURACY_GATE,
    PHASE55_TYPED_EXACT_MATCH_GATE,
...
def _phase54_snapshot() -> dict[str, Any]:
    manifest = _read_json(PHASE54_ROOT / "evidence_manifest.json")
    mismatches = []
    ...
    decision = _read_json(PHASE54_ROOT / "phase54-final-decision.json")
    integrity = _read_json(PHASE54_ROOT / "evidence_integrity.json")
    passed = (
        not mismatches
        and integrity.get("passed") is True
        and decision.get("recommendation") == "hold_phase54_typed_proposition_evaluator"
    )
    return {
        "kind": "phase55_phase54_canonical_snapshot",
        "passed": passed,
        "phase54_recommendation": decision.get("recommendation"),
...
def main() -> int:
    ...
    if args.clean_evidence and EVIDENCE_ROOT.exists():
        preserved = REPO_ROOT / ".phase55-evaluator-debug-preserve"
        if preserved.exists():
    ...
    split = build_phase55_split_integrity(
        calibration["cases"],
        holdout["cases"],
        prior_cases=prior_cases,
        historical_failure_responses=[str(row.get("assistant_response") or "") for row in historical_failures],
    )
    calibration_blind = build_phase55_blind_items(
        calibration["cases"], seed=5501, prefix="phase55-calibration-blind"
    )
    holdout_blind = build_phase55_blind_items(
        holdout["cases"], seed=5502, prefix="phase55-holdout-blind"
    )
    hard_calibration = evaluate_phase55_hard_reject_cases(calibration["cases"])
    hard_holdout = evaluate_phase55_hard_reject_cases(holdout["cases"])
    schema = phase55_ollama_json_schema()
    protocol = {
        "kind": "phase55_frozen_atomic_boundary_protocol",
        "categories": list(PHASE55_CATEGORIES),
        "typed_fields": list(PHASE55_TYPED_FIELDS),
        ...
        "composition": "hard_reject_then_any_registration_elevation_or_asserted_outcome_or_test_transfer_reject_else_all_three_safe_atoms_accept_else_edit",
        "calibration_accuracy_gate": PHASE55_CALIBRATION_ACCURACY_GATE,
        "holdout_accuracy_gate": PHASE55_HOLDOUT_ACCURACY_GATE,
        ...
        "phase51_to_54_holdouts_are_historical_diagnostics_only": True,
    ...
    freeze = {
        "kind": "phase55_pre_model_call_freeze",
        "phase54_canonical_snapshot_passed": baseline["passed"],
        ...
        "extraction_rubric_sha256": stable_hash(PHASE55_EXTRACTION_RUBRIC),
    ...
    historical_snapshot = {
        "kind": "phase55_phase51_to_54_historical_diagnostics",
        "status": "historical_evidence_only",
        "failure_count": len(historical_failures),
        "used_as_phase55_calibration": False,
        "used_as_phase55_holdout": False,
        "used_for_training": False,
        "details": historical_failures,
    }
    source_manifest = {
        "kind": "phase55_source_boundary_manifest",
    ...
    preparation = {
        "kind": "phase55_preparation_decision",
        ...
        "phase54_canonical_passed": baseline["passed"],
        "historical_failure_count": len(historical_failures),
    ...
    no_runtime = {
        "kind": "phase55_runtime_replay_and_boundary_design_status",
        "runtime_replay_status": "not_requested_in_phase55",
        "runtime_replay_model_call_count": 0,
        "boundary_clause_design_status": "not_requested_in_phase55",
        "boundary_clause_design_created": False,
        "runtime_prompt_changed": False,
        "router_changed": False,
    }
    training = {
        "kind": "phase55_training_attempt",
    ...
    _write_json(EVIDENCE_ROOT / "evidence-baseline/phase54_canonical_snapshot.json", baseline)
    _write_json(EVIDENCE_ROOT / "evidence-baseline/phase51_to_54_historical_diagnostics.json", historical_snapshot)
