#!/usr/bin/env python3
"""Run frozen Phase61 compact-wire preflight, calibration, or holdout."""
...
def _invoke_judge(
    *,
    ...
    prompt = build_phase61_wire_judge_prompt(item)
    ...
    try:
        selection = parse_phase61_wire_selection(content, candidates=candidates)
    except ValueError as exc:
        raise JudgeAttemptError(
            f"ValueError: {exc}", raw_response=content or raw_http, failure_class="compact_wire_validation_error"
        ) from exc
    ...
    return {
        ...
        **selection,
        ...
        "compact_wire_valid": True,
        "think": False,
        "temperature": 0,
        "num_ctx": num_ctx,
        "num_predict": num_predict,
        "prompt_sha256": hashlib.sha256(prompt.encode("utf-8")).hexdigest(),
        "wire_spec_sha256": stable_hash(dict(protocol.get("wire_spec") or {})),
...
def _stage_paths(stage: str) -> tuple[Path, Path, Path | None]:
    if stage == "preflight":
        directory = EVIDENCE_ROOT / "evidence-wire-preflight"
...
def _freeze_check(stage: str, public: list[dict[str, Any]], hidden: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    source_ok = (
        ...
        and freeze.get("phase61_evaluator_source_sha256") == _sha256(PHASE61_SOURCE)
    )
    protocol_ok = (
    ...
    if stage == "preflight":
        ...
        prerequisite_ok = _read_json(EVIDENCE_ROOT / "preparation_decision.json").get("status") == "ready_for_wire_preflight"
    else:
        data_ok = (
        ...
        if stage == "calibration":
            prerequisite_ok = _read_json(
                EVIDENCE_ROOT / "evidence-wire-preflight/preflight_report.json"
            ).get("status") == "passed"
        else:
            prerequisite_ok = _read_json(
    ...
    return {
        "kind": "phase61_compact_wire_freeze_check",
        "stage": stage,
...
def main() -> int:
    ...
    if freeze_check["passed"] is not True:
        raise SystemExit(f"Phase61 {args.stage} freeze check failed: {freeze_check}")

    protocol = _read_json(EVIDENCE_ROOT / "evaluator_protocol.json")
    workers = int(protocol.get("parallel_worker_count") or 1)
    if args.workers is not None and args.workers != workers:
        raise SystemExit(f"Phase61 worker count is frozen at {workers}; received --workers={args.workers}")
    ...
    availability = {
        "kind": "phase61_judge_model_availability",
        "stage": args.stage,
    ...
    if availability["passed"] is not True:
        raise SystemExit(f"Phase61 judge models unavailable: {missing}")
    ...
    for alias in aliases:
        ...
        if (output_path.exists() or failure_path.exists()) and not args.resume:
            raise SystemExit(f"Phase61 {args.stage} is frozen one-shot evidence; use --resume only after interruption")
        ...
        for row in existing_results:
            ...
            try:
                parse_phase61_wire_selection(str(row.get("raw_response") or ""), candidates=candidates)
            except ValueError:
                continue
    ...
    if args.stage == "preflight":
        report = {
            "kind": "phase61_compact_wire_preflight_report",
            "stage": args.stage,
