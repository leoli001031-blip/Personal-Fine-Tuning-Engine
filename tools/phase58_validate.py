#!/usr/bin/env python3
"""Run and persist the Phase58 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = json.loads((EVIDENCE_ROOT / "phase58-final-decision.json").read_text(encoding="utf-8"))
    comparison = json.loads((EVIDENCE_ROOT / "comparison_summary.json").read_text(encoding="utf-8"))
    phase58 = dict(comparison.get("phase58") or {})
    qualified = phase58.get("holdout_status") == "qualified"
    expected = (
        "recommend_phase58_clause_addressed_evaluator_for_manual_review_only"
        if qualified else "hold_phase58_clause_addressed_grounding"
    )
    expected_calls = 180 if phase58.get("calibration_status") == "qualified" else 60
    actual_calls = int(phase58.get("calibration_real_model_call_count") or 0) + int(
        phase58.get("holdout_real_model_call_count") or 0
    )
    passed = (
        integrity.get("passed") is True
        and decision.get("recommendation") == expected
        and actual_calls == expected_calls
        and decision.get("runtime_replay_allowed_in_phase58") is False
    ...
    return {
        "name": "phase58_evidence_consistency",
        "command": ["internal", "phase58_evidence_consistency"],
        ...
        "output_tail": [
            ...
            f"calibration={phase58.get('calibration_accuracy')} holdout={phase58.get('holdout_accuracy')} calls={actual_calls}"
        ],
    }
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "pfe-core/pfe_core/phase58_clause_addressed_grounding.py",
                "tools/phase58_prepare.py",
                "tools/phase58_clause_evaluator.py",
                "tools/phase58_finalize_evidence.py",
                "tools/phase58_validate.py",
                "tests/test_phase58_clause_addressed_grounding.py",
            ],
        ),
        ("phase58_focused_and_phase57_to_45_regression", [python, "-m", "pytest", "-q", *regression_tests]),
    ...
    summary = {
        "kind": "phase58_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase58 validation: {summary['status']}"]
    lines.extend(
