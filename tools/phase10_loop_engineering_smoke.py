#!/usr/bin/env python3
"""Run the Phase10 loop-engineered format-training smoke."""
...
def _allocate_workdir(args: argparse.Namespace) -> tuple[Path, bool]:
    ...
    return Path(tempfile.mkdtemp(prefix="pfe-phase10-loop-")), not args.keep_workdir
...
def _maybe_run_qwen_training(args: argparse.Namespace, workdir: Path, preflight: Mapping[str, Any]) -> dict[str, Any]:
    logs_dir = workdir / "phase10-command-logs"
    ...
    if not preflight.get("ready_for_real_training"):
        return {
            ...
            "mock_fallback": False,
            "skip_reason": "phase10_qwen_mlx_preflight_blocked",
...
def _maybe_run_real_eval(args: argparse.Namespace, workdir: Path, training: Mapping[str, Any]) -> dict[str, Any]:
    ...
    store = Phase10LoopEngineeringStore(home=workdir / ".pfe", workspace=args.workspace)
...
def _maybe_run_qwen36_preflight(args: argparse.Namespace) -> dict[str, Any]:
    if not args.run_qwen36_preflight:
        return {}
    return {
        **qwen36_mlx_preflight(
            model_id=args.qwen36_model_id,
            model_path=args.qwen36_model_path,
            require_local_model=args.require_local_qwen36_model,
            allow_remote_download=args.allow_qwen36_remote_download,
            min_memory_gb=args.qwen36_min_memory_gb,
            min_disk_gb=args.qwen36_min_disk_gb,
        ),
        "kind": "phase10_qwen36_preflight",
        "default_run": False,
        "training_allowed": False,
        "reason": "Phase10 only permits Qwen3.6 load/preflight after the small-model gate passes.",
    }
...
def _copy_evidence(args: argparse.Namespace, payload: Mapping[str, Any]) -> dict[str, str]:
    ...
    raw_manifest_path = str(dict(payload.get("training_result", {}).get("training") or {}).get("manifest_path") or "").strip()
    if raw_manifest_path:
        job_result_path = Path(raw_manifest_path).parent / "training_job_result.json"
        if job_result_path.exists():
            target = evidence_dir / "training_job_result.json"
            shutil.copy2(job_result_path, target)
            copied["training_job_result"] = str(target)
    output_path = evidence_dir / "phase10-smoke-output.json"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run Phase10: loop-engineered format curriculum -> Qwen3-0.6B MLX "
            "training attempt -> normalized holdout eval -> Qwen3.6 preflight decision."
        )
    )
    parser.add_argument("--repo-root", type=Path, default=repo_root)
    parser.add_argument("--python", default=_default_python(repo_root))
    parser.add_argument("--workspace", default="phase10_loop_engineering")
    ...
    parser.add_argument("--stage", default=PHASE10_STAGE_A)
    parser.add_argument("--dataset-recipe", default=None)
    parser.add_argument("--model-id", default=PHASE10_RECOMMENDED_MODEL)
    ...
    parser.add_argument("--qwen36-model-id", default=PHASE10_BOUNDARY_MODEL)
    parser.add_argument("--qwen36-model-path", type=Path, default=None)
    parser.add_argument("--run-qwen36-preflight", action="store_true")
    parser.add_argument("--require-local-qwen36-model", action="store_true")
    parser.add_argument("--allow-qwen36-remote-download", action="store_true")
    parser.add_argument("--qwen36-min-memory-gb", type=float, default=64.0)
    parser.add_argument("--qwen36-min-disk-gb", type=float, default=30.0)
    ...
    parser.add_argument("--eval-top-p", type=float, default=0.0)
    parser.add_argument("--eval-repetition-penalty", type=float, default=1.05)
    parser.add_argument(
        "--phase9-eval-path",
        type=Path,
        default=repo_root / "docs/demo/phase9-output-format-training-stability/evidence-real-qwen3-0.6b/eval_report.json",
    )
    parser.add_argument(
        "--phase9-training-job-path",
        type=Path,
        default=repo_root
...
def main() -> int:
    ...
    try:
        ...
        qwen36_preflight = _maybe_run_qwen36_preflight(args)
        finalized = finalize_phase10_loop_experiment(
            ...
            phase9_eval_path=args.phase9_eval_path,
            phase9_training_job_path=args.phase9_training_job_path,
            qwen36_preflight=qwen36_preflight or None,
        )
        payload = {
            "ok": True,
            "workspace": args.workspace,
