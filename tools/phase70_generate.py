#!/usr/bin/env python3
"""Generate real Qwen3-4B transcripts for one frozen Phase70 arm."""
...
def _freeze_check(holdout: Mapping[str, Any], protocol: Mapping[str, Any]) -> dict[str, Any]:
    ...
    source_ok = all(
        _sha256(REPO_ROOT / SOURCE_PATHS[name]) == expected
    ...
    return {
        "kind": "phase70_generation_freeze_check",
        "passed": source_ok
        ...
        "protocol_check_passed": stable_hash(protocol_copy) == protocol.get("protocol_sha256") == freeze.get("protocol_sha256"),
    }
...
def _run_session(
    *,
    ...
    raw_history = [
        {
            "role": "system",
            "content": f"{PHASE46_LENGTH_CONTRACT}\n{PHASE46_LATEST_INTENT_CONTRACT}",
        }
    ]
    persisted_turns = []
    generations = []
    ...
    for turn_index, user_text in enumerate(turns, start=1):
        ...
        runtime = build_phase70_runtime_messages(raw_history, variant=variant)
        ...
        if not cleaned:
            raise RuntimeError("Phase70 output became empty after privacy handling")
    ...
    return {
        "kind": "phase70_real_runtime_ab_transcript",
        ...
        "generation_protocol_sha256": generation_protocol["generation_protocol_sha256"],
        "task_sha256": stable_hash(session),
        "system_contract_sha256": hashlib.sha256(runtime.messages[0]["content"].encode()).hexdigest(),
...
def main() -> int:
    ...
    parser.add_argument("--variant", choices=PHASE70_VARIANTS, required=True)
    ...
    if freeze["passed"] is not True:
        raise SystemExit(f"Phase70 generation freeze failed: {freeze}")
    ...
    by_id = {
        ...
        for row in ([] if args.clean else _read_jsonl(output_path))
    ...
    if runtime.get("adapter_loaded") is not False:
        raise SystemExit("Phase70 must not load an adapter")
    try:
        for index, session in enumerate(sessions, start=1):
            ...
            if row["status"] != "completed" or index % 6 == 0:
                print(f"[{args.variant}] {index}/{len(sessions)} status={row['status']}", flush=True)
    finally:
        del model
    ...
    latencies = [float(value) for row in completed for value in row.get("latency_seconds") or []]
    metrics = {
        "kind": "phase70_real_generation_metrics",
        "variant": args.variant,
