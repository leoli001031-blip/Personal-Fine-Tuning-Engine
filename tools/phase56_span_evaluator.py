#!/usr/bin/env python3
"""Run the frozen Phase56 Gemma/Qwen span-grounded atomic protocol."""
...
def _freeze_check(split: str, public: list[dict[str, Any]], hidden: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    source_ok = (
        ...
        and freeze.get("phase55_evaluator_source_sha256") == _sha256(PHASE55_SOURCE)
    ...
    if split == "calibration":
        ...
        prerequisite_ok = _read_json(
            EVIDENCE_ROOT / "evidence-evaluator-calibration/span_evaluator_report.json"
        ).get("status") == "qualified"
    return {
        "kind": "phase56_span_evaluator_freeze_check",
        "split": split,
...
def main() -> int:
    ...
    if freeze_check["passed"] is not True:
        raise SystemExit(f"Phase56 {args.split} freeze check failed: {freeze_check}")
    ...
    availability = {
        "kind": "phase56_judge_model_availability",
        "split": args.split,
    ...
    if availability["passed"] is not True:
        raise SystemExit(f"Phase56 judge models unavailable: {missing}")
    ...
    _write_json(directory / "span_evaluator_report.json", report)
    print(
        json.dumps(
            {
                key: report.get(key)
