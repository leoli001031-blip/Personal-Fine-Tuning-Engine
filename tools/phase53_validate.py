#!/usr/bin/env python3
"""Run and persist the Phase53 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = json.loads((EVIDENCE_ROOT / "phase53-final-decision.json").read_text(encoding="utf-8"))
    ...
    holdout_qualified = formal.get("holdout_status") == "qualified"
    expected = (
    ...
    holdout_calls_ok = (
        int(formal.get("holdout_real_model_call_count") or 0) == 216
        if formal.get("calibration_status") == "qualified"
        else int(formal.get("holdout_real_model_call_count") or 0) == 0
    )
    passed = (
        integrity.get("passed") is True
        and decision.get("recommendation") == expected
        and int(formal.get("calibration_real_model_call_count") or 0) == 180
    ...
    return {
        "name": "phase53_evidence_consistency",
        "command": ["internal", "phase53_evidence_consistency"],
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "pfe-core/pfe_core/phase53_evaluator_scope_recovery.py",
                "tools/phase53_prepare.py",
                "tools/phase53_dual_evaluator.py",
                "tools/phase53_finalize_evidence.py",
                "tools/phase53_validate.py",
                "tests/test_phase53_evaluator_scope_recovery.py",
            ],
        ),
        (
            "phase53_focused_and_phase52_to_45_regression",
            [
                python,
                "-m",
                "pytest",
    ...
    summary = {
        "kind": "phase53_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase53 validation: {summary['status']}"]
    lines.extend(
