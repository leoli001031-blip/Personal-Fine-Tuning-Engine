#!/usr/bin/env python3
"""Run and persist the Phase59 validation matrix."""
...
def _evidence_check() -> dict[str, Any]:
    ...
    decision = json.loads((EVIDENCE_ROOT / "phase59-final-decision.json").read_text(encoding="utf-8"))
    comparison = json.loads((EVIDENCE_ROOT / "comparison_summary.json").read_text(encoding="utf-8"))
    phase59 = dict(comparison.get("phase59") or {})
    qualified = phase59.get("holdout_status") == "qualified"
    expected = (
        "recommend_phase59_proposition_evaluator_for_manual_review_only"
        if qualified else "hold_phase59_proposition_addressed_grounding"
    )
    expected_outcomes = 180 if phase59.get("calibration_status") == "qualified" else 60
    actual_outcomes = int(phase59.get("calibration_judge_item_outcome_count") or 0) + int(
        phase59.get("holdout_judge_item_outcome_count") or 0
    )
    passed = (
        integrity.get("passed") is True
        and decision.get("recommendation") == expected
        and actual_outcomes == expected_outcomes
        and decision.get("runtime_replay_allowed_in_phase59") is False
    ...
    return {
        "name": "phase59_evidence_consistency",
        "command": ["internal", "phase59_evidence_consistency"],
        ...
        "output_tail": [
            ...
            f"calibration={phase59.get('calibration_accuracy')} holdout={phase59.get('holdout_accuracy')} "
            f"judge_item_outcomes={actual_outcomes}"
        ],
    }
...
def main() -> int:
    ...
    checks = [
        (
            ...
            [
                ...
                "tools/phase59_prepare.py",
                "tools/phase59_candidate_evaluator.py",
                "tools/phase59_finalize_evidence.py",
                "tools/phase59_validate.py",
                "tests/test_phase59_proposition_addressed_grounding.py",
            ],
        ),
        ("phase59_focused_and_phase58_to_45_regression", [python, "-m", "pytest", "-q", *regression_tests]),
    ...
    summary = {
        "kind": "phase59_validation_summary",
        "created_at": _utcnow(),
    ...
    lines = [f"Phase59 validation: {summary['status']}"]
    lines.extend(
