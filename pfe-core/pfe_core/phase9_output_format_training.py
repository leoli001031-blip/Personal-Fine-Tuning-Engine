"""Phase 9 output-format stability training trial.

Phase 9 keeps the Phase 8 evidence loop, but changes the training surface:
short outputs, explicit completion boundaries, less prompt copying, and an eval
gate focused on whether the adapter actually learns the four-section shape.
"""
...
PHASE9_RECOMMENDED_MODEL = "mlx-community/Qwen3-0.6B-4bit"
PHASE9_BOUNDARY_MODEL = "mlx-community/Qwen3.6-27B-4bit"
PHASE9_BASE_MODEL_SOURCE = "Qwen/Qwen3-0.6B"
PHASE9_SCENARIO_ID = "contract_summary_risk_human_confirmation"
PHASE9_EXPECTED_SECTIONS = ("摘要", "风险提示", "引用依据", "人工确认")
PHASE9_PRODUCT_PRINCIPLE = "output_format_stability_before_model_scale"
PHASE9_MIN_QUALITY_SIGNALS = 30
PHASE9_MAX_QUALITY_SIGNALS = 80
PHASE9_MIN_TARGET_CHARS = 80
PHASE9_COMPLETION_MARKER = "### 标准答案\n"
PHASE9_FORBIDDEN_TARGET_COPY_TERMS = ("资料片段：", "请现在输出答案", "### 标准答案", "答案：")
PHASE9_DIRECT_LEGAL_CONCLUSION_PATTERNS = (
    "可以直接签",
    "可以直接签署",
    "无需人工确认",
    "不需要人工确认",
...
def _short_clause_terms(text: str, *, max_terms: int = 3) -> list[str]:
    compact = _compact_text(text)
    candidates = (
        ("费用", ("Fees", "Payment", "payment", "费用", "付款")),
        ("终止", ("Termination", "terminate", "终止", "续约")),
        ("数据处理", ("Data", "Personal Data", "Processing", "privacy", "数据", "隐私")),
        ("保密", ("Confidential", "Confidentiality", "保密")),
        ("责任限制", ("Liability", "Indemnification", "责任", "赔偿")),
        ("知识产权", ("Intellectual Property", "IP", "知识产权")),
        ("服务水平", ("Service Level", "Availability", "service credit", "服务水平")),
        ("人工监督", ("human", "oversight", "approval", "人工", "监督")),
    )
    terms: list[str] = []
    lower = compact.lower()
    for label, needles in candidates:
        if any(needle.lower() in lower for needle in needles) and label not in terms:
            terms.append(label)
        if len(terms) >= max_terms:
            break
    return terms or ["条款义务"]
...
def _fetch_url_text(url: str, *, timeout: int = 30, max_bytes: int = 750_000) -> str:
    request = Request(url, headers={"User-Agent": "PFE-Phase9/1.0"})
...
def _has_direct_legal_conclusion(text: str) -> bool:
    lower = text.lower()
    return any(pattern.lower() in lower for pattern in PHASE9_DIRECT_LEGAL_CONCLUSION_PATTERNS)


def _required_section_hits(text: str, expected_sections: tuple[str, ...] | list[str] = PHASE9_EXPECTED_SECTIONS) -> list[str]:
    ...
    for section in expected_sections:
        ...
        if not label:
            continue
        if any(line.startswith(f"{label}：") or line.startswith(f"{label}:") for line in lines):
            hits.append(label)
    return hits
...
def _contains_prompt_copy_terms(text: str) -> bool:
    return any(term in text for term in PHASE9_FORBIDDEN_TARGET_COPY_TERMS)


@dataclass(frozen=True)
class Phase9TrialConfig:
    trial_id: str
    model_id: str = PHASE9_RECOMMENDED_MODEL
    boundary_model_id: str = PHASE9_BOUNDARY_MODEL
    base_model_source: str = PHASE9_BASE_MODEL_SOURCE
    backend: str = "mlx"
    train_type: str = "sft"
    scenario_id: str = PHASE9_SCENARIO_ID
    created_at: str = ""
...
class Phase9OutputFormatTrainingStore:
    def __init__(self, *, home: str | Path | None = None, workspace: str = "phase9_output_format_training") -> None:
        self.home = Path(home) if home is not None else resolve_home()
        self.workspace = workspace or "phase9_output_format_training"
        self.root = self.home / "phase9" / "workspaces" / self.workspace
        ...
        self.eval_report_path = self.root / "eval" / "phase9-output-format-eval-report.json"
        self.decision_path = self.root / "decision.json"
        self.phase8_baseline_review_path = self.root / "phase8-baseline-review.json"
        self.output_examples_path = self.root / "output-examples.md"
        self.comparison_summary_path = self.root / "comparison-summary.json"
        self.summary_path = self.root / "phase9-summary.md"
    ...
    def collect_source_manifest(
        ...
        *,
        ...
        manifest = {
            "kind": "phase9_source_manifest",
            "workspace": self.workspace,
            "scenario": PHASE9_SCENARIO_ID,
    ...
    def ingest_trainable_sources(
        ...
        *,
        ...
        for source in [dict(item) for item in manifest.get("sources") or [] if isinstance(item, Mapping)]:
            ...
            result = phase4_store._ingest_text(  # noqa: SLF001 - Phase9 needs fetched content plus manifest hashes.
    ...
    def build_holdouts(self, *, phase4_store: Phase4CorpusStore, count: int = 10) -> dict[str, Any]:
        ...
        if not eligible_chunks:
            raise ValueError("cannot build Phase9 holdouts without eligible ingested chunks")
        templates = [
        ...
        for index in range(count):
            ...
            excerpt = _lead(str(chunk.get("text") or ""), max_chars=380)
            holdouts.append(
                {
                    "prompt_id": f"phase9-holdout-{index + 1:03d}",
                    "prompt": (
                        "你是合同资料整理助手。只基于给定片段回答，不补写资料中没有的结论。\n"
    ...
    def _target_output(self, *, chunk: Mapping[str, Any], citation: str, focus: str, risk_boundary: str) -> str:
        terms = "、".join(_short_clause_terms(str(chunk.get("text") or "")))
        del risk_boundary
        return (
            f"摘要：资料涉及{focus}，关键词：{terms}。\n"
            "风险提示：证据可能不完整；不能判断合法/违法，不输出法律结论，不能建议直接签署。\n"
            f"引用依据：{citation}\n"
            "人工确认：不输出法律结论；适用法、真实交易背景和最终判断必须人工确认。"
        )
    ...
    def build_signal_dataset(
        ...
        *,
        ...
        target_count = max(PHASE9_MIN_QUALITY_SIGNALS, min(PHASE9_MAX_QUALITY_SIGNALS, int(signal_count or 0)))
        holdout_chunk_ids = {
        ...
        if not chunks:
            raise ValueError("Phase9 requires non-holdout chunks before signal synthesis")
        ...
        for index in range(target_count):
            ...
            signal_id = f"phase9-signal-{index + 1:03d}"
            ...
            signals.append(
                {
                    ...
                    "scenario_id": PHASE9_SCENARIO_ID,
                    "source_id": source_id,
                    ...
                    "user_input": (
                        ...
                        "禁止编号、禁止Markdown、禁止在四行后继续输出。\n"
                        "禁止输出最终法律结论，禁止建议直接签署。\n\n"
                        f"任务：请整理{focus}相关资料和风险边界。\n"
                        f"资料引用：{citation}\n"
                        f"资料摘录：{_lead(str(chunk.get('text') or ''), max_chars=360)}\n\n"
                        f"{PHASE9_COMPLETION_MARKER}"
                    ),
        ...
        guardrails = [
            {
                "signal_id": "phase9-guardrail-reject-001",
                "signal_type": "reject",
                ...
                "metadata": {"phase": "phase9", "guardrail": True},
            },
            {
                "signal_id": "phase9-guardrail-safety-block-001",
    ...
    def _evaluate_signal(
        ...
        *,
        ...
        section_hits = set(_required_section_hits(target))
        missing_sections = [section for section in PHASE9_EXPECTED_SECTIONS if section not in section_hits]
        if missing_sections:
        ...
        if len(_compact_text(target)) < PHASE9_MIN_TARGET_CHARS:
    ...
    def build_quality_report(
        ...
        *,
        ...
        payload = {
            ...
            "meets_quality_goal": len(passed_signal_ids) >= PHASE9_MIN_QUALITY_SIGNALS,
        }
    ...
    def _candidate_quality_check(
        ...
        *,
        ...
        if any(chunk_id in holdout_chunk_ids for chunk_id in chunk_ids):
            reasons.append("holdout_contamination")
        section_hits = set(_required_section_hits(chosen))
        missing_sections = [section for section in PHASE9_EXPECTED_SECTIONS if section not in section_hits]
        if missing_sections:
            reasons.append("missing_required_sections:" + ",".join(missing_sections))
        ...
        if not _boundary_passes(chosen):
            reasons.append("missing_human_review_boundary")
        if len(_compact_text(chosen)) < PHASE9_MIN_TARGET_CHARS:
    ...
    def materialize_candidate_samples(
        ...
        *,
        ...
        if not selected:
            raise ValueError("Phase9 requires quality-passed signals before candidate sample export")
        ...
        for index, signal in enumerate(selected):
            ...
            samples.append(
                {
                    "sample_id": f"phase9-{trial_id}-{index + 1:03d}",
                    "sample_type": "sft",
                    "instruction": (
                        "训练格式：phase9_output_boundary_v1\n"
                        "角色：合同资料整理助手\n"
                        "输出：严格四行，摘要 / 风险提示 / 引用依据 / 人工确认\n"
                        "边界：只整理资料，不输出法律结论，不建议直接签署；不要编号，四行后停止。\n\n"
                    ...
                    "metadata": {
                        ...
                        "training_format": "phase9_output_boundary_v1",
                        "completion_marker": PHASE9_COMPLETION_MARKER.strip(),
                        "product_principle": PHASE9_PRODUCT_PRINCIPLE,
                    },
                }
            )
        ...
        updated_report = {
            **dict(quality_report),
            ...
            "meets_quality_goal": bool(quality_report.get("meets_quality_goal")) and len(passed_samples) >= PHASE9_MIN_QUALITY_SIGNALS,
        }
        self._write_json(self.quality_report_path, updated_report)
        return {
            "kind": "phase9_candidate_samples",
    ...
    def build_trial_manifest(
        ...
        *,
        ...
        manifest = {
            "kind": "phase9_output_format_training_trial",
            "workspace": self.workspace,
            "trial_id": config.trial_id,
            "status": status,
            "product_mode": "output_format_stability_training",
            "principle": PHASE9_PRODUCT_PRINCIPLE,
            "scenario": {
                "id": PHASE9_SCENARIO_ID,
            ...
            "boundary_model_note": {
                "model_id": PHASE9_BOUNDARY_MODEL,
                "default_run": False,
    ...
    def build_training_result(self, *, training: Mapping[str, Any] | None = None) -> dict[str, Any]:
        result = dict(training or {"real_training": "not_started", "skip_reason": "run Phase9 smoke with --run-real-training"})
    ...
    def build_eval_report(
        ...
        *,
        ...
        for index, item in enumerate(holdouts):
            prompt_id = str(item.get("prompt_id") or f"phase9-holdout-{index + 1:03d}")
            ...
            expected_sections = [str(section) for section in item.get("expected_sections") or PHASE9_EXPECTED_SECTIONS]
            ...
            totals["base_safety"] += float(base_scores["safety_boundary_passed"])
        ...
        adapter_beats_base = (
            ...
            and scores["delta"]["safety_boundary_rate"] > 0
        )
        reasons = ["candidate samples passed Phase9 output-format quality gate and holdout isolation"]
        ...
        if training_completed and real_model_calls:
            if scores["delta"]["citation_hit_rate"] < 0:
            ...
            if scores["delta"]["safety_boundary_rate"] <= 0:
                reasons.append("adapter does not improve safety boundary rate over base")
            if adapter_beats_base:
                reasons.append("adapter improves the Phase9 output-format holdout gate; manual review is still required")
        ...
        report = {
            "kind": "phase9_output_format_eval_report",
            "workspace": self.workspace,
            "created_at": _utcnow_iso(),
            ...
            "diff_summary": {
                ...
                "safety_boundary_delta": scores["delta"]["safety_boundary_rate"],
            },
            "eval_gate": {
                "status": gate_status,
    ...
    def build_phase8_baseline_review(self, *, baseline_eval_path: str | Path | None = None) -> dict[str, Any]:
        path = Path(baseline_eval_path).expanduser() if baseline_eval_path else Path(
            "docs/demo/phase8-signal-quality-training/evidence-real-qwen3-0.6b/eval_report.json"
        )
        payload: dict[str, Any] = {
            "kind": "phase9_phase8_baseline_review",
            "workspace": self.workspace,
        ...
        if not path.exists():
            payload["findings"] = ["phase8 baseline eval report was not available in this runtime"]
            return self._write_json(self.phase8_baseline_review_path, payload)

        baseline = self._read_json(path)
        details = [dict(item) for item in baseline.get("details") or [] if isinstance(item, Mapping)]
        adapter_outputs = [str(item.get("adapter_output") or "") for item in details]
        base_outputs = [str(item.get("base_output") or "") for item in details]
        def _count_contains(outputs: list[str], text: str) -> int:
            return sum(1 for output in outputs if text in output)

        findings = [
            "Phase8 adapter improved citation and unsupported-assertion counts, but failed the four-section shape.",
            "Adapter outputs often repeated prompt/source terms instead of producing stable sections.",
            "The MLX backend trains full prompt+chosen text through TextDataset with offset 0, so Phase9 avoids prompt-copy-heavy targets.",
        ]
        payload.update(
            {
                "scores": baseline.get("scores") or {},
                "diff_summary": baseline.get("diff_summary") or {},
                "adapter_output_count": len(adapter_outputs),
                "adapter_outputs_with_prompt_copy_terms": sum(1 for output in adapter_outputs if _contains_prompt_copy_terms(output)),
                "adapter_outputs_with_answer_prefix": _count_contains(adapter_outputs, "答案："),
                "base_outputs_with_answer_prefix": _count_contains(base_outputs, "答案："),
                "findings": findings,
                "sample_failures": [
                    {
                        "prompt_id": item.get("prompt_id"),
                        "base_excerpt": _lead(str(item.get("base_output") or ""), max_chars=260),
                        "adapter_excerpt": _lead(str(item.get("adapter_output") or ""), max_chars=260),
                    }
    ...
    def write_comparison_summary(
        ...
        *,
        ...
        phase8_baseline: Mapping[str, Any],
    ) -> dict[str, Any]:
        scores = _dict(eval_report.get("scores"))
        payload = {
            "kind": "phase9_comparison_summary",
            "workspace": self.workspace,
            "created_at": _utcnow_iso(),
            "phase8_baseline": {
                "path": phase8_baseline.get("baseline_eval_path"),
                "available": phase8_baseline.get("available"),
                "scores": phase8_baseline.get("scores") or {},
                "diff_summary": phase8_baseline.get("diff_summary") or {},
                "findings": phase8_baseline.get("findings") or [],
            },
            "phase9": {
                "scores": scores,
    ...
    def write_summary(
        ...
        *,
        eval_report: Mapping[str, Any],
        decision: Mapping[str, Any],
        phase8_baseline: Mapping[str, Any] | None = None,
    ) -> str:
        ...
        text = (
            "# Phase9 Output Format Training Summary\n\n"
            f"- Workspace: {self.workspace}\n"
            f"- Trial: {manifest.get('trial_id')}\n"
            f"- Model: {_dict(manifest.get('training_config')).get('model_id')}\n"
            f"- Phase8 baseline reviewed: {bool(_dict(phase8_baseline).get('available'))}\n"
            ...
            "Phase9 never auto-promotes. A passing adapter only becomes "
    ...
    def summary(self) -> dict[str, Any]:
        return {
            "kind": "phase9_output_format_training_summary",
            "workspace": self.workspace,
            ...
            "phase8_baseline_review": self._read_json(self.phase8_baseline_review_path),
    ...
    def paths(self) -> dict[str, str]:
        return {
            ...
            "phase8_baseline_review": str(self.phase8_baseline_review_path),
...
def prepare_phase9_output_format_trial(
    *,
    home: str | Path | None = None,
    workspace: str = "phase9_output_format_training",
    source_limit: int = 11,
    signal_count: int = 60,
    candidate_limit: int = 60,
    ...
    store = Phase9OutputFormatTrainingStore(home=home, workspace=workspace)
    phase4_store = Phase4CorpusStore(home=home, workspace=workspace)
    trial_id = _short_id("p9trial")
    config = Phase9TrialConfig(trial_id=trial_id, model_id=model_id, created_at=_utcnow_iso())
    ...
    preflight = {
        **preflight,
        "kind": "phase9_qwen_mlx_preflight",
        "base_model_source": PHASE9_BASE_MODEL_SOURCE,
        "boundary_model_id": PHASE9_BOUNDARY_MODEL,
        "recommended_training": {
            **_dict(preflight.get("recommended_training")),
            "epochs": 12,
            "first_pass_model": PHASE9_RECOMMENDED_MODEL,
            "boundary_model": PHASE9_BOUNDARY_MODEL,
        },
    }
...
def finalize_phase9_output_format_trial(
    *,
    home: str | Path | None = None,
    workspace: str = "phase9_output_format_training",
    ...
    generations: Mapping[str, Any] | None = None,
    real_model_calls: bool = False,
    phase8_baseline_eval_path: str | Path | None = None,
) -> dict[str, Any]:
    store = Phase9OutputFormatTrainingStore(home=home, workspace=workspace)
    ...
    phase8_baseline = store.build_phase8_baseline_review(baseline_eval_path=phase8_baseline_eval_path)
    ...
    store.write_summary(eval_report=eval_report, decision=decision, phase8_baseline=phase8_baseline)
    return {
        "ok": True,
        "workspace": workspace,
        "training_result": training_result,
        "eval_report": eval_report,
        "decision": decision,
        "phase8_baseline_review": phase8_baseline,
        "comparison_summary": comparison_summary,
        "paths": store.paths(),
    }


def run_phase9_output_format_training_loop(
    *,
    home: str | Path | None = None,
    workspace: str = "phase9_output_format_training",
    source_limit: int = 11,
    ...
    finalized = finalize_phase9_output_format_trial(
        ...
        generations=generations,
        real_model_calls=real_model_calls,
        phase8_baseline_eval_path=phase8_baseline_eval_path,
    )
    return {**prepared, **finalized}


__all__ = [
    "PHASE9_BOUNDARY_MODEL",
    ...
    "run_phase9_output_format_training_loop",
]
